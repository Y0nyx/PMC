...
#======================
dataset_path = "C:/Users/mafrc/Desktop/Uni/PMC/CodePMC/final_dataset/"
input_train_norm = []
input_test_norm = []
...
def build_and_train_model():
    wandb.init(project="local_CAE_trainsing_v0")
    #Building model 
    ...
    predict_sample_and_save(model, input_test_norm[0], input_test_aug_norm[0])


def predict_sample_and_save(model, input_test_norm, input_test_aug_norm):
    
    prediction = model.predict(input_test_aug_norm)

    wandb_save_image_sample(input_test_norm, "Prediction expected output")
    wandb_save_image_sample(input_test_aug_norm, "Prediction input (black square image)")
    wandb_save_image_sample(prediction, "Prediction input")
