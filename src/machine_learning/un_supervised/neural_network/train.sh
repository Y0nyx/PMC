...
readonly TEST_NAME='B_First_HP_Search'
...
cd $PATH_NN
./$SCRIPT_NAME --EPOCHS $EPOCHS --BATCH_SIZE $BATCH_SIZE --LEARNING_RATE $LEARNING_RATE --PATH_RESULTS $PATH_RESULTS --FILEPATH_WEIGHTS $FILEPATH_WEIGHTS --FILEPATH_WEIGHTS_SERCH $FILEPATH_WEIGHTS_SERCH --HP_SEARCH $HP_SEARCH --HP_NAME $HP_NAME --MONITOR_LOSS $MONITOR_LOSS --MONITOR_METRIC $MONITOR_METRIC --MODE_METRIC $MODE_METRIC --VERBOSE --DO_HP_SEARCH --EPOCHS_HP $EPOCHS_HP --NUM_TRIALS_HP $NUM_TRIALS_HP --EXECUTION_PER_TRIAL_HP $EXECUTION_PER_TRIAL_HP --NBEST $NBEST --DATA_PATH $DATA_PATH --MAX_PIXEL_VALUE $MAX_PIXEL_VALUE
