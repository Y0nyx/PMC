...
function apiFrontend(mainWindow,configReact)
{
    ipcMain.on("fetchConfig", (event) => {
      ...
      ipcMain.handle("createClient", async (event,client) => {
       ...
       let _client = {id:uuid,nom:client.nom,telephone:client.telephone,email:client.email}
      ...
      ipcMain.handle("createPiece", async (event,piece) => {
        let uuid = generateUUID()
        let currentDate = new Date().toISOString().replace('T', ' ').replace(/\.\d+Z$/, '');
        let _piece = {id:uuid,date:currentDate,url:piece.url,resultat:piece.resultat,id_client:piece.id_client,id_log:piece.id_log,id_type_piece:piece.id_type_piece,id_erreur_soudure:piece.id_erreur_soudure}
